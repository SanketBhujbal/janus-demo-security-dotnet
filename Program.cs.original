...

{
    ...
    // PCI DSS 3.2: CVV must never be logged; PAN must be masked (first-6 + last-4)
...

{
    ...
    // Reconcile against the authoritative order amount stored in the DB;

    // never trust a monetary value supplied by the caller.

    if (webhook.Amount != order.Amount)

        return Results.BadRequest(new { error = "Amount mismatch: callback value does not match order record." });
...

{
    ...
    // FIXED: ownership check — the authenticated user may only read their own account
    var auth = GetToken(ctx);
    var authenticatedUser = db.Tokens.TryGetValue(auth ?? "", out var u) ? u : null;
    if (authenticatedUser is null) return Results.Unauthorized();

    if (!string.Equals(authenticatedUser, id, StringComparison.Ordinal))
...

{
    ...
    // EXPLOIT: POST /api/payments/transfer twice with the same JSON body; both calls succeed
    //          and return DIFFERENT transaction_ids proving the endpoint accepts replays.
    //          Print EXPLOIT_SUCCESS if tx1_id != tx2_id. — network retry creates duplicate transfer
