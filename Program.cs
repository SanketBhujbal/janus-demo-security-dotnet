...
app.MapPost("/api/auth/login", async (HttpContext ctx, PaymentsStore db) =>
{
    ...
    public ConcurrentDictionary<string, IdempotencyEntry> Idempotency {get;}=new();
...
{
    var auth = GetToken(ctx);
    if (auth is null || !db.Tokens.TryGetValue(auth, out var currentUser)) return Results.Unauthorized();
